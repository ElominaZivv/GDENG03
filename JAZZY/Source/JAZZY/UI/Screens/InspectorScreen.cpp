    ...
    if (m_world.currentSceneState == m_world.EDIT_MODE) {
        ...
        if (ImGui::BeginViewportSideBar("Inspector", viewport, ImGuiDir_Right, 200.0f, windowFlags)) {
            ...
            if (selectedObject->_id != m_world.ROOTSCENE_ID && rb) {
                ...
                if (ImGui::Combo("##RigidBodyType", &selectedBodyType, bodyTypeNames, IM_ARRAYSIZE(bodyTypeNames))) {
                    switch (selectedBodyType) {
                    case 0:
                        rb->setBodyType(BodyType::DYNAMIC);
                        break;
                    case 1: rb->setBodyType(BodyType::KINEMATIC);
                        break;
                    case 2: rb->setBodyType(BodyType::STATIC);
                        break;
                    }
                }
                ImGui::Separator();
