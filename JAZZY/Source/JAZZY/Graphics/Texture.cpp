...
jazzy::Texture::Texture(const TextureDesc& desc, const GraphicsResourceDesc& gDesc): GraphicsResource(gDesc)
{
	if (!desc.size.width) DX3DLogThrowInvalidArg("Width must not be zero.");
	if (!desc.size.height) DX3DLogThrowInvalidArg("Height must not be zero.");
	...
	DX3DGraphicsLogThrowOnFail(m_device.CreateTexture2D(&texDesc, &initData, &m_texture), "CreateTexture2D failed.");
