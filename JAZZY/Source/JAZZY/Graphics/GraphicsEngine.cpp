...
jazzy::GraphicsEngine::GraphicsEngine(const GraphicsEngineDesc& desc) : Base(desc.base)
{
	...
	*/
	...
	Vertex vertexList[] =
	{
		...
		Vertex{ {-cubeSize,cubeSize,-cubeSize}, red},
		Vertex{ {cubeSize,cubeSize,-cubeSize}, green},
		Vertex{ {cubeSize,-cubeSize,-cubeSize}, blue},
		// Back Face
		Vertex{ {cubeSize,-cubeSize,cubeSize}, orange},
		Vertex{ {cubeSize,cubeSize,cubeSize}, red},
		Vertex{ {-cubeSize,cubeSize,cubeSize}, green},
...
void GraphicsEngine::updateConstantData(f32 deltaTime, ConstantData& data, ui32 index)
{
	...
	f32 scaleXY = std::lerp(5.0f, 1.0f, (sin(m_time) + 1.0f) / 2.0f);
	f32 scaleZ = std::lerp(0.01f, 1.0f, (sin(m_time) + 1.0f) / 2.0f);
	worldMat = worldMat * Mat4x4::scale(Vec3(scaleXY, scaleXY, scaleZ));
