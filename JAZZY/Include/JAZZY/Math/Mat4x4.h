...
namespace jazzy
{
	...
	public:
		...
		static Mat4x4 rotateX(const f32 rad) noexcept
		{
			Mat4x4 res{};
			res.m_data[0][0] = 1;
			res.m_data[1][1] = std::cos(rad);
			res.m_data[2][2] = std::cos(rad);
			res.m_data[1][2] = std::sin(rad);
			res.m_data[2][1] = -std::sin(rad);
			res.m_data[3][3] = 1;
			return res;
		}

		static Mat4x4 rotateY(const f32 rad) noexcept
		{
			Mat4x4 res{};
			res.m_data[1][1] = 1;
			res.m_data[0][0] = std::cos(rad);
			res.m_data[2][2] = std::cos(rad);
			res.m_data[2][0] = std::sin(rad);
			res.m_data[0][2] = -std::sin(rad);
			res.m_data[3][3] = 1;
			return res;
		}

		static Mat4x4 rotateZ(const f32 rad) noexcept
		{
			Mat4x4 res{};
			res.m_data[2][2] = 1;
			res.m_data[0][0] = std::cos(rad);
			res.m_data[1][1] = std::cos(rad);
			res.m_data[0][1] = std::sin(rad);
			res.m_data[1][0] = -std::sin(rad);
			res.m_data[3][3] = 1;
			return res;
		}
